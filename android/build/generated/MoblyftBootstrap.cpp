...
#define TAG "ti.moblyft"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Moblyft_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Moblyft.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::MoblyftBindings::lookupGeneratedInit(
...
static void Moblyft_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Moblyft_getBinding)->GetFunction());
}

static void Moblyft_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::MoblyftBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry MoblyftBinding = {
	"ti.moblyft",
	Moblyft_init,
	Moblyft_dispose
};
...
Java_ti_moblyft_MoblyftBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.moblyft", &MoblyftBinding);
	titanium::KrollBindings::addExternalLookup(&(::MoblyftBindings::lookupGeneratedInit));
}
